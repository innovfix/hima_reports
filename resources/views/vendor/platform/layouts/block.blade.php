@php
    // Determine if there is any inner content to render. If not and there is no title/description/commandBar,
    // skip rendering this block to avoid empty white cards.
    $hasInner = false;
    foreach ($manyForms ?? [] as $chunk) {
    ...
    $hasHeader = ! empty($title) || ! empty($description) || ! empty($commandBar);

    if (! $hasInner && ! $hasHeader) {
