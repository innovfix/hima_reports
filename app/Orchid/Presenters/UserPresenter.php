...
class UserPresenter extends Presenter implements Personable, Searchable
{
    ...
    {
        // Ensure we always return a string even if name is null
        return (string) ($this->entity->name ?? (isset($this->entity->id) ? 'User #'.$this->entity->id : 'User'));
    }
    ...
    {
        // If the role pivot table does not exist (we're running against a
        // minimal customer database), avoid querying roles to prevent errors.
    ...
    {
        // If no email is available, return the default avatar directly
